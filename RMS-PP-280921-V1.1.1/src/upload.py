    ...
    try:
        try:
            ...
            if not (data):
                ...
                errlogger.info("Recovery of tracker")
        ...
        if(checkLastUpload(rowLast, hourLast, dayLast,monthLast,yearLast)!=True):
            ...
            with open(parentDir+"/records/tracker.txt", 'w') as rowFile:
                ...
                rowFile.write(data)
                rowFile.close()
        LastUploadRow = rowLast
        LastUploadHour = hourLast
        LastUploadDay = dayLast
        LastUploadMonth = monthLast
        LastUploadYear = yearLast      
    except Exception as e:
...
def updateRowNum(rowUpdate, hourUpdate, dayUpdate, monthUpdate, yearUpdate):
    ...
    try:
        ...
        day     = today.day
        hour    = today.hour
        dateUpdate=str(yearUpdate)+','+str(monthUpdate)+','+str(dayUpdate)+','+str(hourUpdate)+','+str(rowUpdate)
        errlogger.info(dateUpdate)
        if not (hourUpdate>int(hour) or dayUpdate>int(day) or monthUpdate>int(month) or yearUpdate>int(year)):
            with open(parentDir+"/records/tracker.txt", 'w') as rowFile:
            ...
            print("updation data is not currect so updating current date")
            errlogger.info("updation data is not currect so updating current date")
            with open(parentDir+"/records/tracker.txt", 'w') as rowFile:
                #rowFile.write(str(rowUpdate))
                dateUpdate=str(year)+','+str(month)+','+str(day)+','+str(hour)+','+'1'
                print(dateUpdate)
                errlogger.info(dateUpdate)
                rowFile.write(str(dateUpdate))
                ...
                if(conn == False):
                    ...
                    if isGSM==1:
                        if countFalse==10:
                            ...
                            if isGSM==1:
                                if countgsmOFF==5:
                                    #this resets the GSM on every 10 min
                                    print("countgsmOFF is equel to 5 so calling resetGSM")
                                    errlogger.info("countgsmOFF is equel to 5 so calling resetGSM")
                                    countgsmOFF=0
                                    resetGSM()
                                    checkATResponse()
